      ...
      <article className="readerMain">
        ...
        <div className="suttaText">
          ...
          <section className="fullTextSection">
            <div className="fullTextHeader"><div><span className="textSectionLabel">{fullTextNumber} · {vi?'TOÀN VĂN BÀI KINH':'FULL TEXT'}</span><h2>{vi?'Đọc ngay tại thư viện':'Read here in the library'}</h2></div>{fullText&&<small>{vi?'Bản dịch':'Translation'}: {sourceAuthor||fullText.author}</small>}</div>
            {paragraphs.length&&fullText?<PassageCollector segments={fullText.segments} canonicalRef={s.canonicalRef} displayCode={displayCode} title={title} slug={slug} locale={locale}/>:<div className="contentUnavailable"><p>{vi?'Bản toàn văn của ngôn ngữ này hiện chưa có hoặc tạm thời không tải được. Thư viện không tự chuyển sang bản dịch khác.':'The full text for this language is unavailable or temporarily could not be loaded. The library will not silently fall back to another translation.'}</p></div>}
          </section>
