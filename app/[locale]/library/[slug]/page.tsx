...
export default async function SuttaPage({params}:{params:Promise<{locale:string;slug:string}>}){
  ...
  const sourceAuthor=(fullText?.author||'').replace(/^Bhikkhu\s+/i,'').trim();
  const sourceLabel=fullText?`${sourceAuthor||fullText.author} · SuttaCentral`:s.licenseShort;
