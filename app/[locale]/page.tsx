...
export default async function LocaleHome({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<SearchParams>}){
 ...
 const portals=[
   {href:`/${locale}#library`,cls:'sacredBuddha',title:vi?'Đọc kinh':h.sutta,sub:vi?'Mở thư viện và chọn một bài để đọc':h.suttaSub},
   {href:`/${locale}/tam-tang`,cls:'sacredBook',title:vi?'Khám phá Tam Tạng':h.vinaya,sub:vi?'Kinh · Luật · Vi Diệu Pháp trong một hệ thống':h.exploreLead},
   {href:`/${locale}#featured`,cls:'sacredBodhi',title:vi?'Nghe kinh':h.listen,sub:vi?'Mở những bài đã có audio và nghe ngay':h.withAudio}
 ];
 return <main className="homePage polishedHome appHome visualHomeV2 buddhistHome">
   ...
   <section className="shell sacredVisualStrip" aria-label={vi?'Lối vào nhanh':'Quick library portals'}>{portals.map(item=><Link key={item.cls} href={item.href} className={`sacredPortal ${item.cls}`}><span className="sacredPortalShade"/><span className="sacredPortalCopy"><strong>{item.title}</strong><small>{item.sub}</small><em>{vi?'Mở ngay':'Open'} <ArrowRight size={15}/></em></span></Link>)}</section>
