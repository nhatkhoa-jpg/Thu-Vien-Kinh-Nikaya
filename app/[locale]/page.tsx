  ...
  return <main className="homePage polishedHome">
    <section className="hero shell polishedHero">
      <div className="heroCopy">
        ...
        <p className="lede">{vi?'Kinh Tạng, Luật Tạng và Vi Diệu Pháp trong một thư viện hiện đại: tra cứu nhanh, đọc toàn văn, nghe audio, tải tài liệu và theo dõi tiến độ số hóa.':'Sutta, Vinaya and Abhidhamma in one modern library for reading, listening, source verification and long-term preservation.'}</p>
        ...
        <div className="heroStats" aria-label={vi?'Tổng quan dữ liệu thư viện':'Library data overview'}>
          <div><strong>3</strong><span>{vi?'Tạng chính':'Piṭakas'}</span></div>
          <div><strong>{corpus.canonicalCount.toLocaleString(numberLocale)}</strong><span>{vi?'mục canonical đã lập chỉ mục':'canonical references indexed'}</span></div>
          <div><strong>{corpus.vietnameseFullTextCount.toLocaleString(numberLocale)}</strong><span>{vi?'bài có toàn văn tiếng Việt':'Vietnamese full texts'}</span></div>
          <div><strong>{audioReady.toLocaleString(numberLocale)}</strong><span>{vi?'bài có audio':'entries with audio'}</span></div>
        </div>
        <div className="heroTrust"><ShieldCheck size={16}/><span>{vi?'Số liệu lấy từ trạng thái corpus thực tế · Nguồn rõ ràng · Không dùng AI tạo kinh văn':'Live corpus status · Traceable sources · No AI-generated scripture'}</span></div>
      </div>
    ...
    <section className="shell homeStartBand" aria-label={vi?'Bắt đầu sử dụng thư viện':'Start using the library'}>
      ...
      <Link href={`/${locale}/tien-do`}><span className="startIcon"><Activity size={20}/></span><span><strong>{vi?'Tiến độ dữ liệu':'Data progress'}</strong><small>{vi?'Xem corpus đang hoàn thiện đến đâu':'See current corpus coverage'}</small></span><ArrowRight size={17}/></Link>
    </section>

    <section className="shell canonPromo" aria-label={vi?'Tam Tạng Pāli':'Pāli Canon'}>
      <div className="canonPromoCopy"><p className="kicker">{vi?'THƯ VIỆN PHẬT GIÁO THERAVĀDA':'THERAVĀDA LIBRARY'}</p><h2>{vi?'Tam Tạng Pāli là cấu trúc chính của thư viện':'The Pāli Canon is now the library’s main structure'}</h2><p>{vi?'Năm bộ Nikāya là phần Kinh Tạng; Luật Tạng và 7 bộ Vi Diệu Pháp được tổ chức thành các khu riêng và sẽ được bổ sung toàn văn theo nguồn đã xác minh.':'The five Nikāyas sit inside the Sutta Piṭaka, alongside dedicated Vinaya and seven-book Abhidhamma sections.'}</p><Link href={`/${locale}/tam-tang`}>{vi?'Khám phá Tam Tạng Pāli':'Explore the Pāli Canon'} <ArrowRight size={16}/></Link></div>
    ...
    <section className="section shell" id="collections"><div className="sectionHead compactHead"><div><span className="sectionLabel">01</span><h2>{vi?'Khám phá 5 bộ kinh Nikāya':'Five Nikāya collections'}</h2><p>{vi?'Năm bộ Nikāya thuộc Kinh Tạng. Chọn một bộ để đi thẳng vào thư viện và tra cứu toàn văn.':'The five Nikāyas belong to the Sutta Piṭaka. Choose one to browse the discourse library.'}</p></div></div>
    ...
    <footer><div className="shell footerInner"><div><strong>{vi?'Thư Viện Tam Tạng Pāli':d.brand}</strong><small>{vi?'Đọc · Nghe · Tra cứu · Lưu giữ lâu dài':'Read · Listen · Search · Preserve'}</small></div><span>{d.footer}</span></div></footer>
  </main>;
}
