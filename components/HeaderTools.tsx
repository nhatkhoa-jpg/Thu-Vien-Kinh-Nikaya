...
export default function HeaderTools({locale}:{locale:Locale}){
  ...
  return <>
    ...
    {searchOpen&&<div className="commandBackdrop" onMouseDown={()=>setSearchOpen(false)}><section className="commandPalette" onMouseDown={e=>e.stopPropagation()}>
      <div className="commandHead"><Search size={20}/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder={vi?'Tìm TB 21, TrB 1, tên kinh, Pāli, chủ đề...':'Search code, title, Pāli, topic...'}/><button onClick={()=>setSearchOpen(false)} aria-label={vi?'Đóng tìm kiếm':'Close search'}><X size={20}/></button></div>
    ...
    {menuOpen&&<div className="drawerBackdrop" onMouseDown={()=>setMenuOpen(false)}><aside className="mobileDrawer" onMouseDown={e=>e.stopPropagation()} aria-label={vi?'Menu thư viện':'Library menu'}>
      <div className="drawerHead"><strong>5 Đại Tạng Kinh Nikāya</strong><button onClick={()=>setMenuOpen(false)} aria-label={vi?'Đóng menu':'Close menu'}><X size={21}/></button></div>
      <nav>
        <Link onClick={()=>setMenuOpen(false)} href={`/${locale}`}><Home size={19}/>{vi?'Trang chủ':'Home'}</Link>
        <Link onClick={()=>setMenuOpen(false)} href={`/${locale}#library`}><BookOpen size={19}/>{vi?'Tìm & đọc kinh':'Library search'}</Link>
        <Link onClick={()=>setMenuOpen(false)} href={`/${locale}#featured`}><Headphones size={19}/>{vi?'Nghe kinh nổi bật':'Featured listening'}</Link>
        <Link onClick={()=>setMenuOpen(false)} href={`/${locale}/bo-suu-tap`}><BookMarked size={19}/>{vi?'Đoạn kinh đã lưu':'Saved passages'}</Link>
        <Link onClick={()=>setMenuOpen(false)} href={`/${locale}/tien-do`}><Activity size={19}/>{vi?'Tiến độ thư viện':'Library progress'}</Link>
