...
export default function LibraryExplorer({locale,placeholder,initialCollection='ALL'}:{locale:Locale;placeholder:string;initialCollection?:string}){
 ...
 return <div className="explorer">
   ...
   <div className="resultLine"><strong>{filtered.length}</strong> {vi?'bài kinh phù hợp':'matching discourses'}{filtered.length>visible.length&&<span> · {vi?`đang hiện ${visible.length}`:`showing ${visible.length}`}</span>}</div>
   <div className="suttaGrid">{visible.length?visible.map(s=>{const audio=suttaAudio(s,locale);const c=collections.find(x=>x.code===s.collection);const summary=vi?s.summaryVi:s.summaryEn;return <Link href={`/${locale}/library/${s.slug}`} className="suttaCard" key={s.slug}><div className="suttaCode dualCode"><strong>{suttaDisplayCode(s,vi)}</strong>{vi&&<small>{s.code}</small>}</div><div className="suttaContent"><div className="suttaMeta"><span>{vi?c?.vi:c?.en}</span>{s.readMinutes>0&&<span><Clock size={13}/>{s.readMinutes} {vi?'phút':'min'}</span>}</div><h3>{vi?s.vi:s.en}</h3><p className="pali">{s.pali}</p>{summary&&<p className="summary">{summary}</p>}<div className="topicRow">{s.topics.slice(0,3).map(t=><span key={t}>{t}</span>)}</div></div><div className="suttaActions"><span title={vi?'Đọc':'Read'}><BookOpen size={16}/></span>{audio&&<span title={audio.label}><Headphones size={16}/></span>}<ArrowUpRight className="openArrow" size={18}/></div></Link>}):<div className="emptyState"><Search size={26}/><h3>{vi?'Không tìm thấy bài kinh':'No results'}</h3><p>{vi?'Thử TB 21, TrB 1, TƯB 22.59, tên Pāli hoặc một chủ đề.':'Try another code, Pāli title, or broader topic.'}</p></div>}</div>
