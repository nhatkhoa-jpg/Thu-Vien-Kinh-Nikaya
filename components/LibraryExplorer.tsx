...
export default function LibraryExplorer({locale,placeholder,initialCollection='ALL'}:{locale:Locale;placeholder:string;initialCollection?:string}){
 ...
 return <div className="explorer">
   ...
   {selectedNote&&<div className="emptyState collectionStatusCard" style={{marginBottom:'18px'}}><BookOpen size={22}/><h3>{vi?'Tình trạng bộ kinh':'Collection status'}</h3><p>{vi?selectedNote.vi:selectedNote.en}</p></div>}
   <div className="suttaGrid scriptureShelf">{visible.length?visible.map(s=>{const audio=suttaAudio(s,locale);const c=collections.find(x=>x.code===s.collection);const summary=vi?s.summaryVi:s.summaryEn;return <Link href={`/${locale}/library/${s.slug}`} className={`suttaCard scriptureBookCard collection-${s.collection.toLowerCase()}`} key={s.slug}><div className="suttaCoverThumb" style={{backgroundImage:`url(${coverByCollection[s.collection]})`}}><span className="suttaCoverCode">{suttaDisplayCode(s,vi)}</span>{audio&&<span className="suttaCoverAudio"><Headphones size={14}/></span>}</div><div className="suttaContent"><div className="suttaMeta"><span>{vi?c?.vi:c?.en}</span>{s.readMinutes>0&&<span><Clock size={13}/>{s.readMinutes} {vi?'phút':'min'}</span>}</div><h3>{vi?s.vi:s.en}</h3><p className="pali">{s.pali}</p>{summary&&<p className="summary">{summary}</p>}<div className="topicRow">{s.topics.slice(0,3).map(t=><span key={t}>{t}</span>)}</div></div><div className="suttaActions"><span title={vi?'Đọc':'Read'}><BookOpen size={16}/></span>{audio&&<span title={audio.label}><Headphones size={16}/></span>}<ArrowUpRight className="openArrow" size={18}/></div></Link>}):<div className="emptyState"><Search size={26}/><h3>{vi?'Chưa thấy bài phù hợp':'No matching discourse yet'}</h3><p>{vi?'Thử tên kinh, mã số, một từ Pāli hoặc chủ đề rộng hơn.':'Try a title, code, Pāli word, or a broader topic.'}</p></div>}</div>
