...
export type FullTextResult={language:string;author:string;authorUid:string;sourceUrl:string;title?:string;segments:FullTextSegment[]};
...
async function fetchTranslation(uid:string,language:string,preferredAuthor?:string):Promise<FullTextResult|null>{
  try{
    ...
    const title=clean(String(chosen.title||chosen.name||payload?.title||''));
    return {language,author:chosen.author||chosen.author_short||authorUid,authorUid,sourceUrl:`https://suttacentral.net/${uid}/${language}/${authorUid}`,title:title||undefined,segments:normalizeSegments(segments)};
  }catch{return null;}
}

export async function getSuttaFullText(uid:string,locale:Locale):Promise<FullTextResult|null>{
  // Resolve only the language that was explicitly requested here. Any fallback must
  // be chosen by the caller so the UI can label it transparently and never mistake
  // English for the selected language.
  const language=localeMap[locale]||locale;
  return fetchTranslation(uid,language,preferredAuthors[locale]);
}
